(function () {
     ...
     * - сортирует фильмы по рейтингу TMDB;
     * - сортирует фильмы по рейтингу Кинопоиска;
     * - позволяет вернуть исходный порядок CUB;
     * - выбор режима сохраняется отдельно для каждой коллекции;
     * - добавляет пункт сортировки в существующее меню долгого нажатия
     *   на карточке, не заменяя меню других плагинов.
     *
     * Важно:
    ...
    function chooseKpFilm(items, item) {
        ...
        /*
         * Сначала пытаемся сопоставить по IMDb.
         */
        ...
        /*
         * Если кандидатов несколько — дополнительно сверяем год.
         */

        if (cards.length > 1 && year) {
            var byYear = cards.filter(function (c) {
                var y = parseInt(
                    ((c.start_date || c.year || '') + '').slice(0, 4),
                    10
                );
        ...
        /*
         * Используем только однозначное совпадение.
         */
    ...
    function saveKpRating(tmdbId, kp, imdb) {
        ...
        cache[key] = {
            kp: isFinite(parseFloat(kp))
                ? parseFloat(kp)
                : 0,

            imdb: isFinite(parseFloat(imdb))
                ? parseFloat(imdb)
    ...
    function fetchKpRating(item, done) {
            ...
            req.silent(
                ...
                function (data) {
                    ...
                    if (isFinite(kp) && kp > 0) {
                        ...
                        /*
                         * API может вернуть 0/null.
                         * Используем XML как запасной источник.
                         */
                        ...
                        req["native"](
                            ...
                            function (str) {
                                try {
                                    ...
                                    var kpXml =
                                        kpNode.length
                                            ? parseFloat(kpNode.text())
                                            : 0;

                                    var imdbXml =
                                        imdbNode.length
                                            ? parseFloat(imdbNode.text())
                                            : 0;
        ...
        /*
         * Если карточка уже содержит KP ID,
         * поиск по названию не нужен.
         */
        ...
        var cleanTitle = kpCleanTitleForSearch(
            item.title || ''
        );
        ...
        function next() {
            ...
            fetchKpRating(
                ...
                function (rating) {
                     ...
                     * не создаём сотни одновременных
     ...
     *
     * Это нужно для правильной сортировки:
     * фильм с рейтингом 9.0 на странице 2
     * должен быть выше фильма 7.0 на странице 1.
     */
    ...
    function loadAll(
        ...
        function loadPage(page) {
            requestPage(
                ...
                function () {
                    /*
                     * Если последующая страница
                     * не ответила — показываем
     ...
     * ВАЖНО:
     *
     * Мы НЕ вешаем собственный hover:long
     * непосредственно на карточку.
     *
     * Поэтому оригинальный CUB и другие плагины
     * продолжают получать своё обычное меню.
     *
     * Мы только запоминаем факт long press,
     * а затем добавляем свой пункт в уже открываемое
     * Lampa.Select меню.
     */
    ...
    function getCardElementFromEvent(event) {
        var target =
            event && event.target;
    ...
    function installSelectIntegration() {
         ...
         * Capture-фаза.
         *
         * Срабатывает до обработчика CUB.
         * Ничего не отменяем и не останавливаем.
         */
        ...
        /*
         * Перехватываем только момент создания
         * конкретного Select меню после long press.
         */
        ...
        Lampa.Select.show =
            function (params) {
                ...
                /*
                 * Флаг одноразовый.
                 * Следующие меню не должны
                 * автоматически получать наш пункт.
                 */
                ...
                /*
                 * Копируем параметры оригинального меню,
                ...
                /*
                 * Добавляем ОБЫЧНЫЙ активный пункт.
                 *
                 * ВАЖНО:
                 * separator: true здесь НЕТ.
                 */
                ...
                /*
                 * Сохраняем обработчик оригинального меню.
                 */
                ...
                patched.onSelect =
                    function (item) {
                        ...
                        /*
                         * Любой обычный пункт —
                         * отдаём обратно оригинальному
                         * обработчику CUB/другого плагина.
                         */
    ...
    /*
     * Формирует данные для InteractionCategory.
     */
    ...
    function buildCollectionData(
        ...
        /*
         * Передаём InteractionCategory
         * обычный объект ответа CUB.
         */
    ...
    function makeComponent(object) {
        ...
        var collectionId =
            getCollectionId(object);

        var collectionTitle =
            getCollectionTitle(object);

        /*
         * Запоминаем текущую коллекцию.
         *
         * Это используется только для добавления
         * пункта в меню long press.
         */
            ...
            function () {
                ...
                loadAll(
                    ...
                    function (
                         ...
                         * Кинопоиск.
                         *
                         * Сначала получаем отсутствующие
                         * рейтинги в кэш.
                         */
        ...
        /*
         * Lampa может попытаться запросить
         * следующую страницу.
         *
         * Все страницы уже загружены,
         * поэтому ничего дополнительно
    ...
    function startPlugin() {
        ...
        /*
         * CUB Collections уже регистрирует
         * этот component.
         *
         * Мы регистрируем свою версию после него.
         */
        ...
        /*
         * Уведомление показываем один раз.
         */
    ...
    /*
     * Ждём, пока оригинальный CUB Collections
     * создаст Manifest.cub_domain.
     */
