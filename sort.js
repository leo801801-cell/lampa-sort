        ...
        function scan() {
            ...
            if (render && render.find) {
                render.find('.card.selector').each(function () {
                    var cardInstance = $(this).data('card');
                    if (cardInstance) {
                        attachSortMenu(cardInstance, collectionId, title);
                    }
                });
            }
        ...
        comp.create = function () {
            ...
            loadAll(collectionId, function (firstPage, allItems) {
                ...
                if (mode === 'rating_desc') {
                    ...
                    buildResult();
                } else if (mode === 'kp_desc' || mode === 'kp_asc') {
                    self.activity.loader(true);
                    loadKpRatings(allItems, function (ratedItems) {
                        var sorted = ratedItems.sort(function (a, b) {
                            if (a.rating < 0 && b.rating >= 0) return 1;
                            if (a.rating >= 0 && b.rating < 0) return -1;
                            if (a.rating !== b.rating) {
                                return mode === 'kp_asc' ? a.rating - b.rating : b.rating - a.rating;
                            }
                            return 0;
                        });
                        result = sorted.map(function (entry) { return entry.item; });
                        buildResult();
                    });
                } else {
                    buildResult();
